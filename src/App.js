...
class App extends Component {
  ...
  handleNewGame = (...potentialId) => {
    ...
    fetchGameInfo(gameId).then(data => {
      this.setState({
    ...
    fetchGameRun(gameId).then(data => {
      ...
      if (
        data.length > 0 &&
        data[0].runs.length > 0 &&
        data[0].runs[0].run.videos &&
        data[0].runs[0].run.videos.links
      ) {
        this.setState({
          mainSeen: true,
          hasRun: true
  ...
  render() {
    return (
      <div className="home-container">
        ...
        {this.state.mainSeen &&
          !(this.state.hasRun && this.state.hasInfo) &&
